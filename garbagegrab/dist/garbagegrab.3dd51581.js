...
})({"1R540":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "0ace85563dd51581";
"use strict";
...
},{}],"7zBfl":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "SceneManager", ()=>SceneManager);
var _three = require("three");
var _gltfloaderJs = require("three/examples/jsm/loaders/GLTFLoader.js"); // Updated path
class SceneManager {
    ...
    setupLighting() {
        ...
        const ambientLight = new _three.AmbientLight(0x220044, 0.4);
        this.scene.add(ambientLight);
        // Main directional light
        const directionalLight = new _three.DirectionalLight(0x4488ff, 0.8);
        ...
        // Neon accent lights
        const neonLight1 = new _three.PointLight(0xff0088, 1, 20);
        neonLight1.position.set(-15, 5, -15);
        this.scene.add(neonLight1);
        const neonLight2 = new _three.PointLight(0x00ffff, 1, 20);
        neonLight2.position.set(15, 5, 15);
        this.scene.add(neonLight2);
        const neonLight3 = new _three.PointLight(0x88ff00, 1, 20);
    ...
    createArena() {
        ...
        // Main floor
        const floorGeometry = new _three.PlaneGeometry(gridSize, gridSize);
        const floorMaterial = new _three.MeshLambertMaterial({
            color: 0x001122,
            transparent: true,
            opacity: 0.8
        });
        ...
        // Grid lines
        const gridHelper = new _three.GridHelper(gridSize, gridDivisions, 0x00ffff, 0x004488);
    ...
    createSimpleJunk() {
        ...
        for(let i = 0; i < 3 + Math.random() * 3; i++){
            ...
            const geometry = new _three.BoxGeometry(size, size * 0.5, size);
            const material = new _three.MeshLambertMaterial({
                color: new _three.Color().setHSL(0.6, 0.3, 0.2 + Math.random() * 0.3),
                emissive: new _three.Color(0x002244),
                emissiveIntensity: 0.1
            });
            const mesh = new _three.Mesh(geometry, material);
            mesh.position.set((Math.random() - 0.5) * 2, size * 0.25, (Math.random() - 0.5) * 2);
    ...
    createSpikeTraps() {
        ...
        for (const pos of trapPositions){
            ...
            this.spikeTraps.push({
                ...
                mixer: null,
                animations: {},
                position: new _three.Vector3(pos.x, 0, pos.z),
                active: true,
                cooldown: 0,
                isActivated: false
            });
    ...
    createSimpleSpikeTrap() {
        const group = new _three.Group();
        // Base
        const baseGeometry = new _three.CylinderGeometry(0.8, 0.8, 0.1);
        const baseMaterial = new _three.MeshLambertMaterial({
            color: 0x444444,
            emissive: 0x440000,
            emissiveIntensity: 0.2
        });
        const base = new _three.Mesh(baseGeometry, baseMaterial);
        base.castShadow = true;
        base.receiveShadow = true;
        group.add(base);
        // Spikes
        for(let i = 0; i < 6; i++){
            const spikeGeometry = new _three.ConeGeometry(0.1, 1, 6);
            const spikeMaterial = new _three.MeshLambertMaterial({
                color: 0x666666,
                emissive: 0x440000,
                emissiveIntensity: 0.3
            });
            const spike = new _three.Mesh(spikeGeometry, spikeMaterial);
    ...
    checkCollisionWithObstacles(position, radius = 1) {
        ...
        for (const junk of this.junkPiles){
            ...
            if (distance < radius + 1.5) return true;
        }
        return false;
    }
    checkSpikeTraps(position, radius = 1) {
        ...
        for (const trap of this.spikeTraps){
            ...
            if (distance < radius + 0.8) {
                ...
                if (trap.mesh && trap.mesh.children) trap.mesh.children.forEach((child, index)=>{
                    if (index > 0) {
    ...
    cleanup() {
        ...
        if (this.spikeTraps) {
            this.spikeTraps.forEach((trap)=>{
                if (trap.mesh && trap.mesh.parent) this.scene.remove(trap.mesh);
            });
            this.spikeTraps = [];
        }
        // Clean up junk piles
        if (this.junkPiles) {
            this.junkPiles.forEach((junk)=>{
                if (junk.parent) this.scene.remove(junk);
            });
            this.junkPiles = [];
        }
    }
...
},{"three":"dsoTF","three/examples/jsm/loaders/GLTFLoader.js":"t3GCp","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["1R540","7zBfl"], "7zBfl", "parcelRequire482b", {})

//# sourceMappingURL=garbagegrab.3dd51581.js.map
